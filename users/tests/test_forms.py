...
class LoginUserFormTest(TestCase):
    ...
    def test_form_invalid_password(self):
        form = LoginUserForm(data={"username": "user", "password": "invalidpassword"})
        self.assertFalse(form.is_valid())
...
class ChangeUserFormTest(TestCase):
    ...
    def test_form_valid_data(self):
        form = ChangeUserForm(
            instance=self.user, data={"username": "user1", "first_name": "John"}
        )
        self.assertTrue(form.is_valid())
    ...
    def test_form_invalid_username(self):
        form = ChangeUserForm(
            instance=self.user, data={"username": "", "first_name": "John"}
        )
        self.assertFalse(form.is_valid())
...
class ChangeProfileFormTest(TestCase):
    ...
    def test_form_valid_data(self):
        form = ChangeProfileForm(
            ...
            data={"birthday": datetime.date(2000, 12, 15), "city": "Moscow"},
        )
        self.assertTrue(form.is_valid())
