...
class TVSeries(models.Model):
    ...
    poster = models.ImageField(
        upload_to='serials', verbose_name='Постер'
    )
    ...
    actors = models.ManyToManyField(
        'Crew', verbose_name='Актеры', related_name='series_actors'
    )
    directors = models.ManyToManyField(
        'Crew', verbose_name='Режиссеры', related_name='series_directors'
    )
    slug = models.SlugField(
        max_length=255, unique=True, db_index=True, verbose_name='URL'
    )
    favorite = models.ManyToManyField(
        User, blank=True, verbose_name='Избранное', related_name='favorite'
    )
...
class Genre(models.Model):
    ...
    name = models.CharField(
        max_length=100, db_index=True, verbose_name='Жанр'
    )
    slug = models.SlugField(
        max_length=255, unique=True, db_index=True, verbose_name='URL'
    )
...
class Comment(models.Model):
    """Model representing the user's comment on the series."""
